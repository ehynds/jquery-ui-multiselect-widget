...
this.button.outerWidth(width);},_setMenuWidth:function(){var m=this.menu;m.outerWidth(this.button.outerWidth());},_traverse:function(which,start){var $start=$(start);var moveToLast=which===39||which===37;var directionDown=which===40||which===39;var $next=$start.parent()[directionDown?'nextAll':'prevAll']('li:not(.ui-multiselect-disabled, .ui-multiselect-optgroup-label)')[moveToLast?'last':'first']();if(!$next.length){this.menu.find('label')[directionDown?'first':'last']().trigger('mouseover');}else{$next.find('label').trigger('mouseover');}},_toggleState:function(prop,flag){return function(){if(!this.disabled){this[prop]=flag;}
