...
10)-parseInt(b.css("border-left-width"),10);b.width(a||this.button.outerWidth())},_traverse:function(b,a){var e=c(a),d=b===38||b===37;e=e.parent()[d?"prevAll":"nextAll"]("li:not(.ui-multiselect-disabled, .ui-multiselect-optgroup-label)")[d?"last":"first"]();if(e.length)e.find("label").trigger("mouseover");else{e=this.menu.find("ul:last");this.menu.find("label")[d?"last":"first"]().trigger("mouseover");e.scrollTop(d?e.height():0)}},_toggleChecked:function(b,a){var e=a&&a.length?a:this.labels.find("input");
e.not(":disabled").attr({checked:b,"aria-selected":b});this.update();var d=e.map(function(){return this.value}).get();this.element.find("option").filter(function(){return!this.disabled&&c.inArray(this.value,d)>-1}).attr({selected:b,"aria-selected":b})},_toggleDisabled:function(b){this.button.attr({disabled:b,"aria-disabled":b})[b?"addClass":"removeClass"]("ui-state-disabled");this.menu.find("input").attr({disabled:b,"aria-disabled":b}).parent()[b?"addClass":"removeClass"]("ui-state-disabled");this.element.attr({disabled:b,
...
enable:function(){this._toggleDisabled(false)},disable:function(){this._toggleDisabled(true)},checkAll:function(){this._toggleChecked(true);this._trigger("checkAll")},uncheckAll:function(){this._toggleChecked(false);this._trigger("uncheckAll")},getChecked:function(){return this.menu.find("input").filter(":checked")},setChecked:function(b){var a=c.isArray(b)?b.slice():Array(b);this._toggleChecked(false);c.each(this.menu.find("input"),function(e,d){if(a.length===0)return false;var g=c.inArray(c(d).val(),
a);if(g!==-1){c(d).attr("checked",true);a.splice(g,1)}});this.update()},destroy:function(){c.Widget.prototype.destroy.call(this);this.button.remove();this.menu.remove();this.element.show();return this},isOpen:function(){return this._isOpen},widget:function(){return this.menu},_setOption:function(b,a){var e=this.menu;switch(b){case "header":e.find("div.ui-multiselect-header")[a?"show":"hide"]();break;case "checkAllText":e.find("a.ui-multiselect-all span").eq(-1).text(a);break;case "uncheckAllText":e.find("a.ui-multiselect-none span").eq(-1).text(a);
break;case "height":e.find("ul:last").height(parseInt(a,10));break;case "minWidth":this.options[b]=parseInt(a,10);this._setButtonWidth();this._setMenuWidth();break;case "selectedText":case "selectedList":case "noneSelectedText":this.options[b]=a;this.update();break;case "classes":e.add(this.button).removeClass(this.options.classes).addClass(a)}c.Widget.prototype._setOption.apply(this,arguments)}})})(jQuery);
